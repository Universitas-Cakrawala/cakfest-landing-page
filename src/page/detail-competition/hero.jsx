...
import { asset_detail_competition, asset_list_competition } from "../../assets/images";
import {
...
const Hero_Detail_Competitions = () => {
  ...
  const competition = asset_list_competition.find((item) => item.pathName === state?.path);

  if (!competition)
    return (
      <div className="w-full min-h-screen grid place-items-center">
        <div className="text-center space-y-2">
          <h1 className="font-bold text-5xl text-white">Data tidak ditemukan</h1>
  ...
  return (
    <>
      <div className="w-full h-full min-h-screen relative">
        <picture>
          <source media="(max-width: 600px)" srcSet={asset_detail_competition.img_bg_phone} />
          <source media="(max-width: 1000px)" srcSet={asset_detail_competition.img_bg_tablet} />
          <source media="(min-width: 1001px)" srcSet={asset_detail_competition.img_bg} />
          <img
        ...
        <div className="flex flex-col items-center w-full pb-64 md:pb-44 pt-24 sm:pt-44 gap-8 sm:gap-16 z-1 relative px-4 md:px-12 lg:px-24">
          ...
          <div className="hidden sm:flex gap-8 flex-wrap justify-center items-center">
            <Button_Guidebook_detail_competition />
            <Button_Contact_Us_Detail_Competition contact_pic={competition.contact_pic} />
          </div>
        </div>
      </div>
      <Time_Line path={competition.pathName} img_time_line={competition.img_time_line} />
    </>
  );
};
